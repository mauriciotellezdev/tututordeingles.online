...
export async function bookSessionAction(payload: {
  ...
  try {
    ...
    const sessionData = createSession({
      ...
      meetingLink: "https://meet.google.com/hfs-xjqt-wkq",
    ...
    const icsContent = [
      ...
      "LOCATION:Google Meet (https://meet.google.com/hfs-xjqt-wkq)",
      "STATUS:CONFIRMED",
      "SEQUENCE:0",
      "END:VEVENT",
      "END:VCALENDAR"
...
/**
 * Action 4: Create Stripe checkout session or trigger local mock credits
 */
export async function createCheckoutSessionAction(payload: {
  ...
  try {
    ...
    // If Stripe keys are not configured, tell the client to use the mock credit grant flow in dev
    if (!stripeSecretKey) {
      console.log("Stripe API key missing. Returning mock payment trigger.");
      return {
        success: true,
        mockRedirect: true,
        planType
    ...
    const stripe = new Stripe(stripeSecretKey, {
      apiVersion: "2025-01-27.accredited-gratis" as any // fallback to dynamic stripe type config
    });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${appUrl}/student?checkout_success=true&plan=${planType}`,
...
/**
 * Action 5: Mock add credits for local developer testing when Stripe is not configured
 */
export async function /* mockAddCreditsAction removed */(payload: {
  planType: "single" | "package";
}) {
  try {
    const { planType } = payload;
    ...
    console.error("Error in /* mockAddCreditsAction removed */:", error);
    return { success: false, error: error.message || "Error al agregar créditos de simulación." };
  }
}
